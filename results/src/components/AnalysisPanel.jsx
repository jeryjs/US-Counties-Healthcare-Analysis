...
const AnalysisPanel = ({ 
  ...
  const renderRecommendations = () => {
    const getRecommendations = () => {
      if (!filteredCounties.length) return [];
      
      const avgAccess = filteredCounties.reduce((sum, c) => sum + c.Healthcare_Access, 0) / filteredCounties.length;
      const lowAccessCounties = filteredCounties.filter(c => c.Healthcare_Access < 40).length;
      const ruralCounties = filteredCounties.filter(c => c.Population < 50000).length;
      const highPovertyCounties = filteredCounties.filter(c => c.Poverty_Rate > 30).length;

      const recommendations = [];

      if (lowAccessCounties > 0) {
        recommendations.push({
          priority: 'High',
          category: 'Infrastructure',
          title: 'Address Healthcare Deserts',
          description: `${lowAccessCounties} counties have critically low healthcare access scores (<40)`,
          actions: [
            'Establish community health centers',
            'Expand telehealth services',
            'Provide transportation assistance',
            'Recruit healthcare professionals with loan forgiveness'
          ],
          impact: 'Could improve access for ~' + (lowAccessCounties * 25000).toLocaleString() + ' residents'
        });
      }

      if (ruralCounties > filteredCounties.length * 0.5) {
        recommendations.push({
          priority: 'Medium',
          category: 'Rural Health',
          title: 'Rural Healthcare Initiative',
          description: `${ruralCounties} counties are classified as rural with unique challenges`,
          actions: [
          ...
          impact: 'Strengthen healthcare delivery in rural communities'
        });
      }

      if (highPovertyCounties > 0) {
        recommendations.push({
          priority: 'High',
          category: 'Social Determinants',
          title: 'Address Economic Barriers',
          description: `${highPovertyCounties} counties have high poverty rates (>30%) affecting health access`,
          actions: [
          ...
          impact: 'Reduce financial barriers to healthcare'
        });
      }
    ...
    return (
      <div className="space-y-4">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-400">Data-driven policy suggestions for filtered counties</p>
        </div>

        {recommendations.length === 0 ? (
          <div className="text-center text-gray-400 py-8">
            <Target className="w-12 h-12 mx-auto mb-4 opacity-50" />
            <p>No specific recommendations for current filter selection</p>
            <p className="text-sm mt-2">Try expanding your filter criteria</p>
          </div>
        ) : (
          recommendations.map((rec, idx) => (
            <div key={idx} className="bg-gray-800/50 rounded-lg p-4 border border-gray-600">
              ...
              <div className="text-xs text-neon-green bg-green-900/20 rounded p-2">
